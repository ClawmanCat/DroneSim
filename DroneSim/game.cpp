#include "pch.h" // include (only) this in every .cpp file
#include "game.h"
#include "tank.h"
...
Tank& Game::FindClosestEnemy(Tank& current_tank) {
    ...
    for (int i = 0; i < tanks.size(); i++) {
        if (tanks.at(i).allignment != current_tank.allignment && tanks.at(i).active) {
            vec2 l = tanks.at(i).Get_Position() - current_tank.Get_Position();

            float sqrDist = fabsf(l.x * l.x + l.y * l.y);
    ...
    for (Tank& tank : tanks) {
        if (tank.active) {
            ...
            for (Tank& oTank : tanks) {
                ...
                float dirSquaredLen = dir.x * dir.x + dir.y * dir.y;
                ...
                if (dirSquaredLen < colSquaredLen) {
                    tank.Push(glm::normalize(dir), 1.f);
                }
            }
            ...
            if (tank.Rocket_Reloaded()) {
                ...
                rockets.push_back(Rocket(tank.position, glm::normalize(target.Get_Position() - tank.position) * 3.0f, rocket_radius, tank.allignment, ((tank.allignment == RED) ? &rocket_red : &rocket_blue)));
